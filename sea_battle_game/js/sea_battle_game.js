...
/* -------------- ЗАПУСК ИГРЫ -------------- */
		...
		function validatingTheCSSparameterInTheBrowser(prop) {
		...
		function startGame(){
			...
			//Если введенное имя пользователя содержит недопустимые символы или поле пустое,то выводим ошибку и выходим из функции
			...
			generateVariantsForComputerShot();
		...
		function generateVariantsForComputerShot(){
			computer_variants = [];
			computer_variants_coords = [];
			for (var i = 0; i < field_size; i++) {
				var row = []; 
		...
		function addShip(array, ship){			
				...
				for (var j = 0; j < field_size; j++) {
					...
					{	
						...
						for (var k = 0; k < ship.size; k++) {
							...
							if(flag_it_fit == true)
								flag_it_fit = !checkingPresenceOfShipsAlongThePerimeter(array, new_i, new_j, dir, ship.size);
						}
						if(flag_it_fit == true){
							variants.push([i, j]);
						}
			...
			if(variants.length == 0){
				...
				var perimeter = obtainingIndexesOfCellsLocatedAlongThePerimeterOfTheShip(array, location[0], location[1], dir, ship.size);
		...
		/* -------- Функция проверки наличия кораблей по периметру -------- */
		function checkingPresenceOfShipsAlongThePerimeter(array, i, j, dir, ship_size){
			//Получаем индексы ячеек по периметру
			var perimeter = obtainingIndexesOfCellsLocatedAlongThePerimeterOfTheShip(array, i, j, dir, ship_size),
				flag_exist_ship = false;

			//Перебираем все ячейки по периметру
			for (var i = 0; i < perimeter.length; i++) {
				//Проверка ячейки на наличие корабля
				flag_exist_ship = presenceOfShipInCell(array, perimeter[i][0], perimeter[i][1]);
		...
		/* -------- Функция получения индексов ячеек, расположенных по периметру корабля */
		function obtainingIndexesOfCellsLocatedAlongThePerimeterOfTheShip(array, i, j, dir, ship_size){
		...
		/* -------- Функция проверки наличия корабля в ячейке -------- */
		function presenceOfShipInCell(array, i, j){
			//Если ячейка выходит за края поля, то 
		...
		/* -------- Функция установки значения ячейки, если ячейка существует -------- */
...
/* --------------- ХОД ИГРЫ --------------- */
		...
		$(document).on('click','#computer-field td:not(.hit, .missed)',function() {
			...
			var td = $(this);
			blockPlayer();
			shotOnSpecificCell(td, computer_td, computer_array, false);
		});


		/* -------- Функция выстрела по определенной ячейке -------- */
		function shotOnSpecificCell(td, array_td, array, computer_event){
		...
		function hit(td, array_td, array, row, cell, computer_event){
			...
			if(validatingTheCSSparameterInTheBrowser('pointer-events'))
			...
			{	
				...
				var perimeter = obtainingIndexesOfCellsLocatedAlongThePerimeterOfTheShip(array, shipwreck[0][0], shipwreck[0][1], dir, ship_size);
			...
			//Если корабль не уничтожен
				...
				if(computer_event == true){
					...
					if(computer_td_direction == -1){
						...
						if(checkCellForshotOnSpecificCell(computer_variants, row + 1, cell) == true)
							computer_hit_variants.push([row + 1, cell]);
						
						if(checkCellForshotOnSpecificCell(computer_variants, row - 1, cell) == true)
							computer_hit_variants.push([row - 1, cell]);

						if(checkCellForshotOnSpecificCell(computer_variants, row, cell + 1) == true)
					...
					if(computer_td_direction == 0){
						...
						if(checkCellForshotOnSpecificCell(computer_variants, left_coord[0] , left_coord[1] - 1) == true)
							computer_hit_variants.push([left_coord[0] , left_coord[1] - 1]);

						if(checkCellForshotOnSpecificCell(computer_variants, right_coord[0], right_coord[1] + 1) == true)
					...
					if(computer_td_direction == 1){
						...
						if(checkCellForshotOnSpecificCell(computer_variants, top_coord[0] - 1, top_coord[1]) == true)
							computer_hit_variants.push([top_coord[0] - 1, top_coord[1]]);

						if(checkCellForshotOnSpecificCell(computer_variants, bottom_coord[0] + 1, bottom_coord[1]) == true)
		...
		function checkCellForshotOnSpecificCell(array, i, j){
			if(i < 0 || j < 0 || i >= field_size || j >= field_size)
				return false;
			else{
		...
		/* -------- Функция выбора ячейки для атаки компьютером --------  */
		function computerStep(){
			setTimeout(function(){
				...
				if(computer_hit_variants.length == 0){
					...
					setTimeout(function(){
						shotOnSpecificCell(td, player_td, player_array, true);
					}, 200);
				}
				else{
		...
		/* -------- Функция получения элемента td по заданным координатам -------- */
		function getTD(array_td, row, cell){
			return $(array_td[row * field_size + cell]);
		}


		/* -------- Функция получения  координаты элемента td в массиве -------- */
		...
		/* -------- Функция удаления варианта по заданным координатам */
		...
		function endGame(){
			setTimeout(function(){
				...
				$('#game_score').html(score_game[0] + ' : ' + score_game[1]);
			}, 1000);
		}
	});
})(jQuery)